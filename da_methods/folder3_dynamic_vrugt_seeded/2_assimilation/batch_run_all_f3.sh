#!/usr/bin/env bash
# Folder 3 — Dynamic Vrugt seeded: full 2_assimilation pipeline
#
# Runs 3 stages for all 21 catchments:
#   Stage A: 2a/2b perturbation arms  (run_perturbation_da_on.py, Vrugt R, seed=42)
#   Stage B: 2c 18hr forecast cycles  (run_lead_time_forecast_sweep.py, Vrugt R, seed=42)
#   Stage C: 2d 600-member ensemble   (run_crossed_ensemble.py, Vrugt R, seed=42)
#
# Usage:
#   bash batch_run_all_f3.sh            # runs all 3 stages
#   bash batch_run_all_f3.sh arms       # stage A only
#   bash batch_run_all_f3.sh forecast   # stage B only
#   bash batch_run_all_f3.sh ensemble   # stage C only
...
# Paths
DA_SRC="/mnt/disk2/1400_sites_helene/da_results_dynamic_vrugt_seeded"
...
OUT_ARMS="/mnt/disk2/1400_sites_helene/da_arms_dynamic_vrugt_seeded"
OUT_FORECAST="/mnt/disk2/1400_sites_helene/da_forecast_dynamic_vrugt_seeded"
OUT_ENSEMBLE="/mnt/disk2/1400_sites_helene/da_crossed_dynamic_vrugt_seeded"

LOG_DIR="$HOME/f3_logs"
mkdir -p "$LOG_DIR"
...
run_arms() {
  echo "[F3] Stage A: perturbation arms (Vrugt R, seed=42)"
...
run_forecast() {
  echo "[F3] Stage B: 18hr forecast cycles (Vrugt R, seed=42)"
  for cat_id in $CATS; do
    params="$DA_SRC/$cat_id/${cat_id}_best_params.json"
    [ -f "$params" ] || { echo "  [skip] $cat_id — no params"; continue; }
...
run_ensemble() {
  echo "[F3] Stage C: 600-member crossed ensemble (Vrugt R, seed=42)"
  for cat_id in $CATS; do
    params="$DA_SRC/$cat_id/${cat_id}_best_params.json"
    [ -f "$params" ] || { echo "  [skip] $cat_id — no params"; continue; }
