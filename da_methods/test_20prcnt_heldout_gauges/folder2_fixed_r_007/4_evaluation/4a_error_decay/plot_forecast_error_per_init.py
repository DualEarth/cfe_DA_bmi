...
Plotted as:
    DA  : thin colored lines (one per init time, colored by date) + thick mean across all
...
Outputs:
    ...
    <out-dir>/forecast_rmse_per_init_helene.png    (absolute error / RMSE per init)
"""
...
def build_error_table(df, obs_series, init_start, init_end):
    """Return DataFrame: issue_time, lead_hour, ens_mean, obs, error."""
...
def plot_error(ax, err_df, color, alpha_thin, lw_thin, linestyle, label_prefix):
    """Plot individual init-time error curves + thick mean curve."""
    ...
    for t0, grp in err_df.groupby("issue_time"):
        ...
        # Align to leads grid — some may be missing
...
def main():
    ...
    ax.set_title(
        ...
        "DA (purple solid) vs Open-loop (gray dashed) | Sep 24–28 2024 | USGS 03463300",
        fontsize=11,
    )
    ...
    # ---- Absolute error (|error|) averaged per lead — cleaner summary ----
    fig, ax = plt.subplots(figsize=(12, 6))
    ...
    ax.set_title(
        ...
        "DA (purple) vs Open-loop (gray) | Sep 24–28 2024 | USGS 03463300",
        fontsize=11,
    )
