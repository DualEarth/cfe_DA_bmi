...
def build_fixed_target_errors(df, obs_series, target_times):
    """
    For each target time T and each lead L (1..18):
        issue_time = T - L hours
        error = ensemble_mean at (issue_time, lead=L) - obs(T)
    Returns dict: target_time -> {lead: error}
    """
    # Index df by (issue_time, lead_hour) for fast lookup
...
def plot_spaghetti(ax, error_dict, color_da, label_prefix, linestyle="-", lw=0.9, alpha=0.35):
    """Plot one thin line per target time + thick mean."""
...
def main():
    ...
    print("Building fixed-target error tables...")
    ...
    # ---- Spaghetti: per-target-time curves, DA vs OL ----
    ...
    plot_spaghetti(ax_da, da_errors, "tab:purple", "DA", linestyle="-")
    ...
    fig.suptitle(
        ...
        f"Target times: {TARGET_START.strftime('%b %d %H UTC')} → "
    ...
    # ---- Summary: mean across all target times, DA vs OL overlaid ----
    ...
    ax.fill_between(leads, da_mean - da_std, da_mean + da_std,
                    color="tab:purple", alpha=0.15, zorder=2)
    ax.fill_between(leads, ol_mean - ol_std, ol_mean + ol_std,
                    color="tab:gray",   alpha=0.15, zorder=2)
    ax.plot(leads, da_mean, color="tab:purple", lw=2.6, marker="o",
    ...
    ax.set_title(
        ...
        f"Target: {TARGET_START.strftime('%b %d %H UTC')} → "
        f"{TARGET_END.strftime('%b %d %H UTC')}  |  "
        "Lead 1 = init 1 hr before target  |  Lead 18 = init 18 hr before target",
        fontsize=11,
    )
