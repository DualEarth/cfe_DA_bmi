#!/usr/bin/env bash
# F2 fixed R=0.07 (20pct gauge holdout) — 4c reconstructed timeseries plots.
#
# Reads routed_leadtime_da_full.parquet + routed_leadtime_openloop_full.parquet
# from the F2 leadtime route dir and writes output PNGs there.
# Must run AFTER route_leadtime_f2.sh.
#
# Usage:
#   bash run_4c_f2.sh
...
ROUTE_DIR=/mnt/disk2/suma_helen_poster/leadtime_troute_routing_f2
...
echo "[F2-4c] Reconstructed timeseries from: $ROUTE_DIR"
$TROUTE "$SCRIPT_DIR/plot_lead_time_reconstructed_timeseries.py" \
    --route-dir "$ROUTE_DIR" \
    --usgs-csv  "$USGS_CSV"

echo "[F2-4c] Forecast spaghetti..."
$TROUTE "$SCRIPT_DIR/plot_forecast_spaghetti.py" \
    --route-dir "$ROUTE_DIR" \
    --usgs-csv  "$USGS_CSV"

echo "[F2-4c] Helene vs low-flow issue-time hydrograph (per catchment)..."
for CAT in "${CATS[@]}"; do
...
echo "[F2-4c] Done. Outputs in: $ROUTE_DIR and $LEADTIME_DIR"
