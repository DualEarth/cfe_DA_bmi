...
def run(args, bmi_cfe, EnKFAssimilator):
    ...
    enkf = EnKFAssimilator(
        ...
        obs_file=obs_file,
        use_vrugt_r=(not args.no_vrugt_r) and (not getattr(args, 'direct_variance', False)),
    ...
    if getattr(args, 'direct_variance', False):
        obs_df = pd.read_csv(obs_file)
        t_col = next(c for c in obs_df.columns if c.lower() in ('datetime', 'date', 'time', 'timestamp'))
        ...
        print(f"[per-member] obs_var_dict overridden: R = σ² direct ({len(enkf.obs_var_dict)} timesteps)")
...
def main():
    ...
    parser.add_argument('--direct-variance',    action='store_true', default=False,
                        help='Use kriging variance column as R per timestep (R=σ²; overrides Vrugt formula)')
