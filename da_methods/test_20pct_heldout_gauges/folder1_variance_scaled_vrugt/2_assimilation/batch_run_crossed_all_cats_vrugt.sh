#!/bin/bash
# batch_run_crossed_all_cats_vrugt.sh
# Runs run_crossed_ensemble.py for all 21 catchments using dynamic Vrugt R:
#   R(t) = (0.10 * y_obs(t))^2 + 0.001 * krig_var(t)
#
# Outputs go to v2_crossed_ensemble_vrugt/ (separate from the R=0.07 ensemble).
# Run from the server:
#   nohup bash ~/batch_run_crossed_all_cats_vrugt.sh \
#       > ~/crossed_vrugt.log 2>&1 &
...
OUT_DIR=/mnt/disk2/suma_helen_poster/da_results/v2_crossed_ensemble_vrugt
BEST_PARAMS_SRC=/mnt/disk2/suma_helen_poster/da_results/v2_true_enkf_vrugt
...
PYTHON=/home/svyas/miniconda3/envs/troute/bin/python
...
for CAT in "${CATS[@]}"; do
    ...
    # Stage best_params from v2_true_enkf_vrugt (Folder 1 analysis results)
    if [ ! -f "$CAT_OUT/${CAT}_best_params.json" ]; then
        SRC="$BEST_PARAMS_SRC/$CAT/${CAT}_best_params.json"
        if [ -f "$SRC" ]; then
            cp "$SRC" "$CAT_OUT/"
            echo "  Staged best_params from v2_true_enkf_vrugt"
        else
    ...
    # No --hardcoded-r flag → default=None → uses Vrugt dynamic R
    $PYTHON ~/run_crossed_ensemble.py \
        --cat-id "$CAT" \
