...
DEFAULT_ARM_DIR  = "/mnt/disk2/suma_helen_poster/da_results/v2_perturbation_da_on"
...
def load_arm(path):
    """Load arm CSV, compute valid_time, member columns."""
    df = pd.read_csv(path)
...
def plot_spaghetti_arm(ax, df, member_cols, arm_color, obs_series,
    ...
    """
    Plot one thin trajectory per (issue_time, member) pair.
    Each trajectory's x = valid_time values for that issue_time.
    """
    _add_helene_band(ax)
    ...
    for t0 in issue_times:
        ...
        mem_vals = sub_w[member_cols].to_numpy(dtype=float)  # shape (T, N_mem)
        if "mm" not in label_stem.lower():
            mem_vals = mem_vals * MM_H_TO_M3_S  # mm/h -> m³/s

        # Min-max envelope + median line
    ...
    # Overall mean across all issue times (interpolated to common grid)
    if all_means:
...
def compute_envelope(df, member_cols):
    """
    Aggregate all members across all issue_times by valid_time.
    Returns DataFrame indexed by valid_time with columns: q_min, q_med, q_max.
    """
    records = []
...
def main():
    ...
    # ------------------------------------------------------------------ #
    # Figure 2a: Forcing arm spaghetti                                    #
    # ------------------------------------------------------------------ #
    ...
    # ------------------------------------------------------------------ #
    # Figure 2b: Hydro-state arm spaghetti                                #
    # ------------------------------------------------------------------ #
    fig, ax = plt.subplots(figsize=(17, 6))
    plot_spaghetti_arm(
        ax, df_ha, ha_cols,
    ...
    # ------------------------------------------------------------------ #
    # Figure 2ab: Comparison — envelopes overlaid, both arms              #
    # ------------------------------------------------------------------ #
    ...
    if not env_fa.empty:
        ax.fill_between(env_fa["valid_time"],
                        env_fa["q_min"], env_fa["q_max"],
    ...
    if not env_ha.empty:
        ax.fill_between(env_ha["valid_time"],
                        env_ha["q_min"], env_ha["q_max"],
