#!/usr/bin/env bash
# F3 dynamic Vrugt seeded (20pct gauge holdout) — 4a lead-time decay plots.
#
# Runs plot scripts per catchment:
#   plot_lead_time_decay.py          — pooled RMSE vs lead (catchment-level)
#   plot_lead_time_decay_by_regime.py — same split by flow regime
#
# Gauge-level decay plots run once if routed parquets exist:
#   plot_lead_time_decay_gauge.py    — gauge-level (requires routed parquets)
#   plot_forecast_error_fixed_target.py
#   plot_forecast_error_per_init.py
#
# Usage:
#   bash run_4a_f3.sh
...
LEADTIME_DIR=/mnt/disk2/1400_sites_helene/da_forecast_dynamic_vrugt_seeded
DA_DIR=/mnt/disk2/1400_sites_helene/da_arms_dynamic_vrugt_seeded
OBS_DIR=/mnt/disk2/1400_sites_helene/catchment_ts_03463300_spliced_dyn_helene
ROUTE_DIR=/mnt/disk2/suma_helen_poster/leadtime_troute_routing_f3
...
echo "[F3-4a] Lead-time decay plots — leadtime dir: $LEADTIME_DIR"
...
# Gauge-level decay (needs routed parquets — run after route_leadtime_f3.sh)
if [ -f "$ROUTE_DIR/routed_leadtime_da_full.parquet" ]; then
    echo "[F3-4a] Gauge-level decay (routed)..."
    $TROUTE "$SCRIPT_DIR/plot_lead_time_decay_gauge.py" \
        --route-dir "$ROUTE_DIR" \
        --usgs-csv  "$USGS_CSV"

    echo "[F3-4a] Fixed-target error plots..."
    $TROUTE "$SCRIPT_DIR/plot_forecast_error_fixed_target.py" \
        --route-dir "$ROUTE_DIR" \
        --usgs-csv  "$USGS_CSV"

    echo "[F3-4a] Per-init error plots..."
    $TROUTE "$SCRIPT_DIR/plot_forecast_error_per_init.py" \
        --route-dir "$ROUTE_DIR" \
        --usgs-csv  "$USGS_CSV"
else
    echo "[F3-4a] Routed parquets not found — skipping gauge-level and error plots."
    echo "         Run route_leadtime_f3.sh first, then re-run this script."
fi
