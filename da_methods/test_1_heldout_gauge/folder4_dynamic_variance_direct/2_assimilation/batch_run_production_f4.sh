#!/usr/bin/env bash
# Batch production per-member run — F4 Direct variance, 1 gauge holdout.
# Runs run_production_per_member.py for all 21 catchments with --direct-variance.
# Skips if output CSV already exists.
#
# Usage:
#   nohup bash batch_run_production_f4.sh > ~/logs/production_f4.log 2>&1 &
...
echo "F4 production per-member (direct variance) — out: $OUT_DIR"
echo ""
...
echo ""
echo "=== F4 production per-member done: done=$DONE  skipped=$SKIP  failed=$FAIL ==="
