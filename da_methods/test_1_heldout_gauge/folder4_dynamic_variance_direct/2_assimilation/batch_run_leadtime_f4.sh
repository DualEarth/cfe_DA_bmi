#!/usr/bin/env bash
# Batch lead-time forecast sweep — F4 direct variance, 1 gauge holdout.
# Runs run_lead_time_forecast_sweep.py for all 21 catchments with --direct-variance.
# Skips if both output CSVs already exist.
#
# After this completes, run route_lead_time_forecasts.py to route through T-route.
#
# Usage:
#   nohup bash batch_run_leadtime_f4.sh > ~/logs/leadtime_f4.log 2>&1 &
...
echo "F4 lead-time sweep (direct variance) — out: $OUT_DIR"
echo ""
...
echo ""
echo "=== F4 lead-time sweep done: done=$DONE  skipped=$SKIP  failed=$FAIL ==="
