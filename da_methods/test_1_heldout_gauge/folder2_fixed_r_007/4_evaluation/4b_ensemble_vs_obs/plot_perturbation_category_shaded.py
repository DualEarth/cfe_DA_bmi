...
F2 fixed R=0.07 (1 gauge holdout).
...
Inputs (existing per-source sensitivity CSVs from run_perturbation_sensitivity.py):
    ...
    <OBS-DIR>/<CAT>.csv                             (Qkrig obs, gapfilled)
...
SEN_DIR = "/mnt/disk2/suma_helen_poster/da_results_1gauge_heldout/folder2_fixed_r007"
...
def main():
    ...
    fig.suptitle(
        f"Ensemble forecast by perturbation category — {CAT} — F2 (fixed R=0.07)\n"
    ...
    fig.suptitle(
        f"Ensemble forecast by perturbation category — {CAT} — F2 (fixed R=0.07) — log scale\n"
        "Sep 20 - Oct 5, 2024 (Hurricane Helene window) | "
        "Shaded bands = min-max envelope across 20 members. Lines = ensemble median.",
        fontsize=11, y=0.995,
    )
