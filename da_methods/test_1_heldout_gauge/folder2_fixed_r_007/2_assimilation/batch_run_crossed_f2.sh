#!/usr/bin/env bash
# Batch crossed ensemble for all 21 catchments — F2 fixed R=0.07.
# Reads best_params from the F2 DA out dir (already staged).
# Outputs _crossed_ensemble.parquet per catchment to the same dir.
#
# Usage:
#   nohup bash ~/da_1gauge_f2/2_assimilation/batch_run_crossed_f2.sh > ~/crossed_f2.log 2>&1 &
...
OUT_DIR=/mnt/disk2/suma_helen_poster/da_results_1gauge_heldout/folder2_fixed_r007
...
echo "F2 Fixed R=0.07 crossed ensemble — out: $OUT_DIR"
echo ""
...
echo ""
echo "=== F2 crossed done: done=$DONE  skipped=$SKIP  failed=$FAIL ==="
