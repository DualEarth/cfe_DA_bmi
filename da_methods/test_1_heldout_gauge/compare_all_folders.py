...
(South Toe River near Celo, NC), one curve per R-formula experiment:

    Open loop — No DA
    Folder 1  — Variance scaled Vrugt   R(t)=(0.10*y)^2 + 0.001*sigma2_krig
    Folder 2  — Fixed R=0.07            R=0.07 (constant)
    Folder 4  — Dynamic variance direct R(t)=sigma2_krig
    Folder 5  — Re-kriged variance      R(t)=sigma2_krig (re-kriged network)

Reads routed_Q_test.csv (columns: date, Q_routed_m3s, Q_usgs_m3s) from each
folder. Skips any folder where the CSV does not exist.
...
Usage:
    python3 compare_all_folders.py
    python3 compare_all_folders.py --out-dir /path/to/out
"""
...
BASE = "/mnt/disk2/suma_helen_poster/da_results_1gauge_heldout"
...
FOLDERS = [
    {
        ...
        "lw": 1.4,
        "linestyle": "--",
        "csv": f"{BASE}/openloop/routed_Q_test.csv",
    },
    {
        ...
        "lw": 2.0,
        "linestyle": "-",
        "csv": f"{BASE}/folder1_vrugt/routed_Q_test.csv",
    },
    {
        ...
        "lw": 1.6,
        "linestyle": "-",
        "csv": f"{BASE}/folder2_fixed_r007/routed_Q_test.csv",
    },
    {
        ...
        "lw": 1.4,
        "linestyle": "-",
        "csv": f"{BASE}/folder4_dynamic_variance_direct/routed_Q_test.csv",
    },
    {
...
def plot_comparison(dfs, obs, dates, helene_mask, out_path, helene_only=False):
    ...
    for cfg, df in zip(FOLDERS, dfs):
        ...
        obs_sel  = obs[helene_mask] if helene_only else obs
        kg = kge(obs_sel, sim_plot)
        ns = nse(obs_sel, sim_plot)
        ax.plot(plot_dates, sim_plot,
                color=cfg["color"], lw=cfg["lw"],
                linestyle=cfg.get("linestyle", "-"), zorder=4,
    ...
    ax.set_title(title + "DA R-formula comparison at gauge 03463300\n"
                 "(1-gauge holdout, test Oct 2023 – Oct 2024)", fontsize=12)
...
def main():
    ...
    ref = next((df for df in dfs if df is not None), None)
    if ref is None:
        raise RuntimeError("No routed CSVs found — run routing scripts first.")
    dates  = ref.index
    ...
    for cfg, df in zip(FOLDERS, dfs):
        ...
        rows.append({
            ...
            "helene_peak_m3s": round(float(np.nanmax(sim[helene])), 1),
        })
