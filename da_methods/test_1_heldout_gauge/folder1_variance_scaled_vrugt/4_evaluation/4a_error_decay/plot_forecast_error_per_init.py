"""
plot_forecast_error_per_init.py — F1 variance-scaled Vrugt (1 gauge holdout).
...
def main():
    ...
    plot_error(ax, da_err, color="tab:blue",  alpha_thin=0.18, lw_thin=0.8,
    ...
    ax.set_title(
        ...
        "F1 (variance-scaled Vrugt) | DA (blue solid) vs Open-loop (gray dashed) | "
    ...
    ax.plot(da_mae.index, da_mae.values,
            color="tab:blue", lw=2.4, marker="o", label="DA — mean |error|")
    ...
    ax.set_title(
        ...
        "F1 (variance-scaled Vrugt) | DA (blue) vs Open-loop (gray) | "
        "Sep 24–28 2024 | USGS 03463300",
        fontsize=11,
    )
