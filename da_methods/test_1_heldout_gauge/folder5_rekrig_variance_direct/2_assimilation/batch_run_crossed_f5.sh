#!/usr/bin/env bash
# Batch crossed ensemble for all 21 catchments — F5 re-kriging variance.
# R = krig_var directly (re-kriged obs variance); uses --direct-variance flag.
# Outputs _crossed_ensemble.parquet per catchment.
#
# Usage:
#   nohup bash batch_run_crossed_f5.sh > ~/logs/crossed_f5.log 2>&1 &
...
echo "F5 Re-kriging variance crossed ensemble — out: $OUT_DIR"
echo ""
...
echo ""
echo "=== F5 crossed done: done=$DONE  skipped=$SKIP  failed=$FAIL ==="
