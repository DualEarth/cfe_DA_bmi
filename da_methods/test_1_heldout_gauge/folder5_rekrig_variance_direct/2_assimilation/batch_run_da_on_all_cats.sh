#!/usr/bin/env bash
# Batch DA for all 21 catchments — F5 re-kriging variance.
# R = krig_var directly (re-kriged sigma^2 from Qkrig, no Vrugt scaling).
...
mkdir -p "$OUT_DIR"
echo "F5 Re-kriging variance DA — obs: $OBS_DIR"
echo "                            out: $OUT_DIR"
echo ""
...
echo ""
echo "=== F5 done: done=$DONE  skipped=$SKIP  failed=$FAIL ==="
