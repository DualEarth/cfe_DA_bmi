...
#    def update(self):
  ...
  #      perturb_these_forcings = ['atmosphere_water__time_integral_of_precipitation_mass_flux', 'water_potential_evaporation_flux']
    
   #     for perturb_this_forcing in perturb_these_forcings:
    ...
    #    self.current_ensemble_member = 0
     #   for forcing_ens in range(self.perturb_forcings["N"]):
        ...
        # This perturbs the forcing from the saved unperturbed values
      #      for perturb_this_forcing in perturb_these_forcings:
...
#        return np.dtype(self.get_var_type(name)).itemsize
    ...
    def perturb_forcing_from_unperturbed_value(self, var_name):
        ...
        #if var_name == "atmosphere_water__time_integral_of_precipitation_mass_flux":
         #   error_factor_preci = 0.01
        #    perturbed_forcing = np.random.lognormal(0.5 * np.log(unperturbed_value / (error_factor_preci + 1)),       np.sqrt(np.log(error_factor_preci**2 + 1))) * unperturbed_value
            
       # elif var_name == "water_potential_evaporation_flux":
      #      perturbation_factor_et = np.random.standard_normal()
     #       error_factor_et = 0.01
        
    #        perturbed_forcing = (perturbation_factor_et * unperturbed_value * error_factor_et) + unperturbed_value
   #     else:
  #          print('no input')
 #       self._values[var_name] = perturbed_forcing
    ...
    def set_ensemble_member_pet(self):
        var_name = "water_potential_evaporation_flux"
        self.E_potential_et_m_per_s[self.current_ensemble_member] = self._values[var_name]
        
    #-------------------------------------------------------------------
    def perturb_cfe_states(self):
        ens = self.current_ensemble_member
        
   #     self.E_soil_reservoir[ens]['storage_m']  =self.E_soil_reservoir[ens]['storage_m'] 
    #    self.E_gw_reservoir[ens]['storage_m'] = self.E_gw_reservoir[ens]['storage_m']
   ...
   #     self.E_soil_reservoir[ens]['storage_m'] =self.E_soil_reservoir[ens]['storage_m'] 
    #    self.E_gw_reservoir[ens]['storage_m'] =  self.E_gw_reservoir[ens]['storage_m']
        ...
        error_factor_sm = 0.002
        
        self.E_soil_reservoir[ens]['storage_m'] = (perturbation_factor_sm * self.E_soil_reservoir[ens]['storage_m'] * error_factor_sm) + self.E_soil_reservoir[ens]['storage_m']
        
        perturbation_factor_gw = np.random.standard_normal()
        error_factor_gw = 0.0015
        
        self.E_gw_reservoir[ens]['storage_m'] = (perturbation_factor_gw * self.E_gw_reservoir[ens]['storage_m'] * error_factor_gw) + self.E_gw_reservoir[ens]['storage_m']       
